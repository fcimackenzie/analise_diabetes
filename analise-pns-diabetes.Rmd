...
diab_censit <- svytable(~ q030 + v0026, pns_design_srvyr)
diab_censit.df <- as.data.frame(diab_censit)
diab_censit.df <- diab_censit.df[diab_censit.df$q030 == '1',]
diab_censit.df$v0026 <- c("Urbano", "Rural")
diab_censit.df$Freq <- diab_censit.df$Freq / sum(diab_censit.df$Freq)

p <-ggplot(data=diab_censit.df, aes(x=v0026, y=Freq*100)) +
  geom_bar(stat="identity", fill="steelblue") +
  geom_text(aes(label=round(Freq*100, digits = 1)), vjust=-0.5, color="black", size=3.5)+
  theme_minimal()
p
